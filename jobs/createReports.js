...
module.exports = (fn) => db(cn => {
    ...
    sql = "INSERT INTO ad_reports (id, day) "
        ...
        + "(SELECT * FROM ad_reports WHERE ad_reports.id = ads.id AND "
        + "ad_reports.day = DATE_ADD(CURDATE(), INTERVAL 1 DAY))";
    ...
    cn.query(sql, (err, result) => {
        ...
        sql = "INSERT INTO pub_reports (id, day) "
            ...
            + "WHERE NOT EXISTS (SELECT * FROM pub_reports WHERE "
            + "pub_reports.id = pubs.id AND "
            + "pub_reports.day = DATE_ADD(CURDATE(), INTERVAL 1 DAY))";
        cn.query(sql, (err, result) => {
            cn.release();
