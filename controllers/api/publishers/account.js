    ...
    register: function (req, res) {
        if (!req.session.uid)
            ...
            db(function (connection) {
                connection.query("SELECT * FROM awaiting_publishers WHERE user_id = ?", [req.session.uid], function (err, rows) {
                    if (rows.length > 0) {
                        ...
                        connection.query("INSERT INTO awaiting_publishers SET ?", data, function (err, result) {
                            connection.release();
                            if (err)
